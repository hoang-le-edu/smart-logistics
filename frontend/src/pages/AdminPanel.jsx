...
export default function AdminPanel({ account, chainId }) {
  ...
  const [editRole, setEditRole] = useState("SHIPPER");
  const loadGranted = async () => {
    try {
      ...
      const decoded = logs.map((l) => {
        ...
        const roleName = roleHex === defaultAdminRole
          ? "ADMIN"
          : roleHex === shipperRole
          ? "SHIPPER"
          : roleHex === carrierRole
      ...
      const withNames = await Promise.all(decoded.map(async (g) => {
        let name = "";
        let senderName = "";
        try { name = await registry.displayName(g.account); } catch {}
        try { senderName = await registry.displayName(g.sender); } catch {}
      ...
      const roleChecks = await Promise.all(
        unique.map(async (u) => ({
          u,
          has: await registry.hasRole(roleHexMap[u.role] || defaultAdminRole, u.account),
        }))
      );
  ...
  const onGrantRole = async () => {
    ...
    if (granteeName.trim()) {
      try {
        await run(() => registry.setDisplayNameFor(address, granteeName.trim()));
      } catch (e) {
        console.warn("setDisplayNameFor failed", e);
        setStatus("Không thể lưu tên cho người được gán. Kiểm tra quyền admin hoặc hàm trên contract.");
      }
    }

    if (role === "SHIPPER") await run(() => registry.grantShipperRole(address));
    else if (role === "CARRIER") await run(() => registry.grantCarrierRole(address));
    else if (role === "BUYER") await run(() => registry.grantBuyerRole(address));
  ...
  const onRevokeRole = async () => {
    ...
    const ROLE = role === "SHIPPER" ? await registry.SHIPPER_ROLE()
      : role === "CARRIER" ? await registry.CARRIER_ROLE()
      : role === "BUYER" ? await registry.BUYER_ROLE()
      : null;
  ...
  const getRoleBytes = async (registry, roleStr) => {
    if (roleStr === "SHIPPER") return registry.SHIPPER_ROLE();
  ...
  const onSaveEdit = async () => {
    ...
    try {
      ...
      if ((editName || "").trim() && editName.trim() !== (g.name || "").trim()) {
      ...
      if (g.role !== "ADMIN" && editRole !== g.role) {
        ...
        if (editRole === "SHIPPER") await run(() => registry.grantShipperRole(g.account));
        else if (editRole === "CARRIER") await run(() => registry.grantCarrierRole(g.account));
        else if (editRole === "BUYER") await run(() => registry.grantBuyerRole(g.account));
      }
  ...
  const onUpdateGrantCard = async (idx) => {
    ...
    try {
      ...
      if (g.role !== "ADMIN" && g.originalRole && g.originalRole !== g.role) {
        ...
        if (g.role === "SHIPPER") await run(() => registry.grantShipperRole(g.account));
        else if (g.role === "CARRIER") await run(() => registry.grantCarrierRole(g.account));
        else if (g.role === "BUYER") await run(() => registry.grantBuyerRole(g.account));
      }
      await loadGranted();
      setStatus("Updated successfully");
    } catch (e) {
  ...
  <div className="p-6 mx-auto max-w-lg" style={{ color: "#000" }}>
    <h2 className="text-2xl font-semibold mb-6 text-center" style={{ color: "#000" }}>
    ...
    <div className="mt-6">
      <h3 className="font-semibold mb-3 text-center" style={{ color: "#000" }}>
        Recently Granted Roles
      </h3>

      <div className="space-y-3">
        {grants.map((g, i) => (
          <div key={i} className="rounded-lg border border-gray-200 shadow px-4 py-3 bg-white text-black">
            <div className="flex items-start gap-4">
              <div className="flex-1 space-y-1">
                <div><strong>Role:</strong> {g.role}</div>
                <div className="break-all"><strong>Account:</strong> {g.account}</div>
                <div><strong>Name:</strong> {g.name || "(no name set)"}</div>
                <div className="text-xs text-gray-700">granted by {g.sender} {g.senderName ? `(${g.senderName})` : ""}</div>
              </div>
              <div className="flex flex-col gap-2">
                <button className="action-button" onClick={() => openEditModal(i)}>Update</button>
                <button className="action-button" disabled={g.role === "ADMIN"} onClick={() => onRevokeGrantCard(i)}>Revoke Role</button>
              </div>
            </div>
          </div>
        ))}
          ...
          {editIndex >= 0 && (
            <>
              <div className="mb-3">
                ...
                <div className="text-sm break-all">{grants[editIndex].account}</div>
              </div>
              <div className="mb-3">
              ...
              <div className="mb-4">
                ...
                <input className="form-input" value={editName} onChange={(e) => setEditName(e.target.value)} placeholder="(no name set)" />
              </div>
              <div className="flex justify-end gap-3">
                <button className="action-button" onClick={closeEditModal}>Cancel</button>
                <button className="action-button primary" onClick={onSaveEdit}>Save</button>
              </div>
            </>
          )}
        </div>
